...
const PSQL_PATH = path.join(PG_BIN, 'psql.exe');
const SHP2PGSQL = `"${SHP2PGSQL_PATH}"`;
const PSQL = `"${PSQL_PATH}"`;
const OGR2OGR = 'ogr2ogr';

function ensurePostgresTooling() {
  const missing = [SHP2PGSQL_PATH, PSQL_PATH].filter((toolPath) => !fs.existsSync(toolPath));
  if (missing.length) {
    throw new Error(
...
function splitQualifiedLayerName(layerName) {
  ...
  const normalizeUploadTableName = (value) => {
    const table = sanitizeLayerName(value, 'upload_layer');
    return table.endsWith('_edit') ? table.slice(0, -5) : table;
  };
  if (parts.length >= 2) {
    return {
      schema: sanitizeLayerName(parts[0], config.UPLOADS.DEFAULT_SCHEMA || 'sde'),
      table: normalizeUploadTableName(parts[1]),
    };
  }
  return {
    schema: null,
    table: normalizeUploadTableName(layerName),
  };
}
...
function autoMapColumns(sourceColumns, targetColumns) {
  ...
  for (const target of targetColumns) {
    ...
    if (sourceCol) {
      ...
      const autoFillExpr = targetName === 'globalid'
        ? (
          target.data_type === 'uuid'
...
async function ensureGeneratedGlobalIds(client, schema, table, geomColumn) {
  await client.query(
    `ALTER TABLE "${schema}"."${table}" ADD COLUMN IF NOT EXISTS "__upload_globalid" text`,
  );
  ...
  for (const row of rows) {
    await client.query(
      `UPDATE "${schema}"."${table}"
       SET "__upload_globalid" = $1
       WHERE ctid = $2`,
      [generateGUID(), row.ctid],
    );
  }
}
...
async function migrateTempToSde(tempSchema, tempTable, sdeSchema, sdeTable, isNew) {
  ...
  try {
    ...
    await ensureGeneratedGlobalIds(client, tempSchema, tempTable, sourceGeomCol);
